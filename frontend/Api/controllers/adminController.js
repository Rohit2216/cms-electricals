...
const updateAdminProfile = async(req, res) => {
    ...
    try 
    {
       ...
       const {error} = adminCreateValidation.validate({email: req.body.email, password: password, contact_no: req.body.contact_no})
        ...
        const updateQuery = `UPDATE admins SET name = ?, email = ?, contact_no = ?, alt_number = ?, address_1 = ?, status = ?, country = ?, city = ?, pin_code = ?, image = ?, description = ?, gst_number = ?, fb_url = ?, inst_url = ?, twitter_url = ?, linkedin_url = ?, updated_at = ? WHERE id= ?`
