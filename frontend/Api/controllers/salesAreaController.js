...
const getSalesArea = async (req, res) => {
    ...
    try 
    {
        ...
        const countSelectQuery = `SELECT COUNT(*) as total FROM sales_area`
        ...
        db.query(selectQuery, (err, result) => {
            ...
            if(result.length > process.env.VALUE_ZERO)
            {
                ...
                return res.status(200).json({status: true, message: 'Sales area fetched successfully', data: result, pageDetails: pageDetails});
            }
            else
            {
                return res.status(400).json({status: false, message: "Something went wrong, please try again."});
