...
const { calculatePagination, getUserDetails, getCreatedByDetails, getLastBalanceOfAccount } = require('../helpers/general');
...
const getTransferFund = async (req, res) => {
    ...
    try {
        ...
        const selectQuery = `SELECT new_fund_requests.* FROM new_fund_requests WHERE status IN ('4','5') AND total_transfer_amount IS NOT NULL AND total_transfer_amount <> 0 ${search_value} ORDER BY new_fund_requests.id ASC LIMIT ${pageFirstResult} , ${pageSize}`;
...
const getALLTransferFund = async (req, res) => {
    ...
    try {
        ...
        const selectQuery = `SELECT new_fund_requests.* FROM new_fund_requests WHERE total_approved_amount IS NOT NULL AND total_approved_amount <> 0 AND status IN ('1', '4', '5')  ${search_value} ORDER BY new_fund_requests.id ASC LIMIT ${pageFirstResult} , ${pageSize}`;
