...
const getExpenseRequest = async (req, res) => {
    try {
        ...
        //     search_value += `AND (users.name LIKE '%${searchData}%' OR users.username LIKE '%${searchData}%' OR users.employee_id LIKE '%${searchData}%')`;
        // }

        // thats correct sql query 12:29pm 27-03-2024
        // const selectQuery = `SELECT request_by, SUM(item_price * request_qty) AS total_approve_amount, DATE_FORMAT(request_date, '%Y-%m') AS request_month, SUM(expense_transfer_amounts) as transfer_amounts FROM fund_requests WHERE request_date < CURRENT_DATE AND request_date >= DATE_SUB(CURRENT_DATE, INTERVAL 1 YEAR) GROUP BY request_by, request_month ORDER BY request_month DESC, request_by ASC LIMIT ${pageFirstResult} , ${pageSize}`
        ...
        // const selectQuery = `
        //     SELECT request_by, SUM(item_price * request_qty) AS total_approve_amount, DATE_FORMAT(request_date, '%Y-%m') AS request_month, SUM(expense_transfer_amounts) AS transfer_amounts FROM fund_requests LEFT JOIN users ON fund_requests.request_by = users.id LEFT JOIN admins ON fund_requests.request_by = admins.id WHERE request_date < CURRENT_DATE AND request_date >= DATE_SUB(CURRENT_DATE, INTERVAL 1 YEAR) ${search_value} GROUP BY request_by, request_month ORDER BY request_month DESC, request_by ASC LIMIT ${pageFirstResult}, ${pageSize}
        // `;

        // const selectQuery = `SELECT request_by, SUM(item_price * request_qty) AS total_approve_amount, DATE_FORMAT(request_date, '%Y-%m') AS request_month, SUM(expense_transfer_amounts) AS transfer_amounts, COALESCE(users.username, admins.name) AS user_name FROM fund_requests LEFT JOIN users ON fund_requests.request_by = users.id ${search_value} LEFT JOIN admins ON fund_requests.request_by = admins.id ${search_values} WHERE (request_date < CURRENT_DATE AND request_date >= DATE_SUB(CURRENT_DATE, INTERVAL 1 YEAR) AND (users.username IS NOT NULL OR admins.name IS NOT NULL)) GROUP BY request_by, request_month ORDER BY request_month DESC, request_by ASC LIMIT ${pageFirstResult}, ${pageSize}; `
