...
const getAllStoredEmployeeDetails = async (req, res) => {
  try {
    ...
    const selectQuery = `SELECT users.id, users.name, users.employee_id, users.email, users.mobile, users.joining_date, users.image, users.status, users.user_type,roles.name as role_name,  users.created_by  FROM users LEFT JOIN roles ON roles.id = users.user_type  WHERE  (users.is_deleted = '0' AND users.created_by ='${user_id}')  ${searchDataCondition} ORDER BY id DESC LIMIT ?, ?`;
